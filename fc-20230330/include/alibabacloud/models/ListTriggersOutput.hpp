...
namespace Models
{
  ...
  public:
    ...
    inline const vector<Trigger> & triggers() const { DARABONBA_PTR_GET_CONST(triggers_, vector<Trigger>) };
    inline vector<Trigger> triggers() { DARABONBA_PTR_GET(triggers_, vector<Trigger>) };
  ...
  protected:
    ...
    std::shared_ptr<vector<Trigger>> triggers_ = nullptr;
  };
