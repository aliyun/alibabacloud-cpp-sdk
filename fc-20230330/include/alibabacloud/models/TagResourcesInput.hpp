...
namespace Models
{
  ...
  public:
    ...
    inline const vector<Tag> & tag() const { DARABONBA_PTR_GET_CONST(tag_, vector<Tag>) };
    inline vector<Tag> tag() { DARABONBA_PTR_GET(tag_, vector<Tag>) };
  ...
  protected:
    ...
    // This parameter is required.
    std::shared_ptr<vector<Tag>> tag_ = nullptr;
  };
