...
public:
  ...
  explicit VerifyIntelligentCaptchaRequest(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  ...
  explicit VerifyIntelligentCaptchaResponseBodyResult(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  ...
  shared_ptr<string> message{};
  shared_ptr<string> requestId{};
  shared_ptr<VerifyIntelligentCaptchaResponseBodyResult> result{};
  shared_ptr<bool> success{};

  VerifyIntelligentCaptchaResponseBody() {}

  explicit VerifyIntelligentCaptchaResponseBody(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  void fromMap(map<string, boost::any> m) override {
    ...
    if (m.find("Result") != m.end() && !m["Result"].empty()) {
      if (typeid(map<string, boost::any>) == m["Result"].type()) {
        VerifyIntelligentCaptchaResponseBodyResult model1;
        model1.fromMap(boost::any_cast<map<string, boost::any>>(m["Result"]));
        result = make_shared<VerifyIntelligentCaptchaResponseBodyResult>(model1);
      }
    }
    if (m.find("Success") != m.end() && !m["Success"].empty()) {
      success = make_shared<bool>(boost::any_cast<bool>(m["Success"]));
...
public:
  shared_ptr<map<string, string>> headers{};
  shared_ptr<long> statusCode{};
  shared_ptr<VerifyIntelligentCaptchaResponseBody> body{};

  VerifyIntelligentCaptchaResponse() {}

  explicit VerifyIntelligentCaptchaResponse(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  void fromMap(map<string, boost::any> m) override {
    ...
    if (m.find("body") != m.end() && !m["body"].empty()) {
      if (typeid(map<string, boost::any>) == m["body"].type()) {
        VerifyIntelligentCaptchaResponseBody model1;
        model1.fromMap(boost::any_cast<map<string, boost::any>>(m["body"]));
        body = make_shared<VerifyIntelligentCaptchaResponseBody>(model1);
      }
    }
  }
...
public:
  ...
  VerifyIntelligentCaptchaResponse verifyIntelligentCaptchaWithOptions(shared_ptr<VerifyIntelligentCaptchaRequest> request, shared_ptr<Darabonba_Util::RuntimeOptions> runtime);
  VerifyIntelligentCaptchaResponse verifyIntelligentCaptcha(shared_ptr<VerifyIntelligentCaptchaRequest> request);
