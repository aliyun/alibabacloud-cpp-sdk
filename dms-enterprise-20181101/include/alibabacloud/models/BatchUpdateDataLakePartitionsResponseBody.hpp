...
namespace Models
{
  ...
  public:
    ...
    inline const vector<PartitionError> & getPartitionErrors() const { DARABONBA_PTR_GET_CONST(partitionErrors_, vector<PartitionError>) };
    inline vector<PartitionError> getPartitionErrors() { DARABONBA_PTR_GET(partitionErrors_, vector<PartitionError>) };
  ...
  protected:
    ...
    // The error messages.
    shared_ptr<vector<PartitionError>> partitionErrors_ {};
