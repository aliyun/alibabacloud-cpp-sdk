...
/**
 * @summary Retrieves files and their details from a specified knowledge base.
 *
 ...
 */
ListPublishedAgentResponse Client::listPublishedAgentWithOptions(const string &workspaceId, const ListPublishedAgentRequest &tmpReq, const map<string, string> &headers, const Darabonba::RuntimeOptions &runtime) {
  tmpReq.validate();
  ListPublishedAgentShrinkRequest request = ListPublishedAgentShrinkRequest();
  Utils::Utils::convert(tmpReq, request);
  if (!!tmpReq.hasSubTypes()) {
    request.setSubTypesShrink(Utils::Utils::arrayToStringWithSpecifiedStyle(tmpReq.getSubTypes(), "subTypes", "json"));
  }
  ...
  if (!!request.hasSubTypesShrink()) {
    query["subTypes"] = request.getSubTypesShrink();
  }
...
/**
 * @summary Appends parsed files to a specified knowledge base.
 *
 * @description <props="china">
