...
namespace Models
{
  ...
  public:
    ...
    virtual bool empty() const override { return this->clientNodeConfiguration_ == nullptr
        ...
        && this->nodeSpec_ == nullptr && this->updateType_ == nullptr && this->warmNodeConfiguration_ == nullptr && this->clientToken_ == nullptr && this->force_ == nullptr
        && this->orderActionType_ == nullptr; };
    ...
    inline UpdateInstanceRequest& setUpdateType(string updateType) { DARABONBA_PTR_SET_VALUE(updateType_, updateType) };
    ...
    inline string getOrderActionType() const { DARABONBA_PTR_GET_DEFAULT(orderActionType_, "") };
  ...
  protected:
    ...
    // normal: in-place change
    // blue_green: blue-green change
    shared_ptr<string> updateType_ {};
    // The warm node configuration. The warm node is different from the data node.
    ...
    // 
    // - true: Forcibly restarts the instance.
    // - false (default): Does not forcibly restart the instance.
    shared_ptr<bool> force_ {};
    // The type of configuration change. Valid values:
    // 
    // - upgrade (default): upgrade
    // - downgrade: downgrade
    shared_ptr<string> orderActionType_ {};
  };
