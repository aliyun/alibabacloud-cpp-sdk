...
namespace Models
{
  ...
  public:
    ...
    virtual bool empty() const override { return this->CPUUsageRate_ == nullptr
        ...
        && this->userNumber_ == nullptr && this->tasks_ == nullptr; };
    ...
    inline const vector<Task> & getTasks() const { DARABONBA_PTR_GET_CONST(tasks_, vector<Task>) };
    inline vector<Task> getTasks() { DARABONBA_PTR_GET(tasks_, vector<Task>) };
    inline QuotaNodeViewMetric& setTasks(const vector<Task> & tasks) { DARABONBA_PTR_SET_VALUE(tasks_, tasks) };
    inline QuotaNodeViewMetric& setTasks(vector<Task> && tasks) { DARABONBA_PTR_SET_RVALUE(tasks_, tasks) };
