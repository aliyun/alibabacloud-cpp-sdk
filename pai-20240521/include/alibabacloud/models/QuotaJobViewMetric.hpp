...
namespace Models
{
  ...
  public:
    ...
    virtual bool empty() const override { return this->CPUUsageRate_ == nullptr
        ...
        && this->userId_ == nullptr && this->task_ == nullptr; };
    ...
    inline const Task & getTask() const { DARABONBA_PTR_GET_CONST(task_, Task) };
    inline Task getTask() { DARABONBA_PTR_GET(task_, Task) };
    inline QuotaJobViewMetric& setTask(const Task & task) { DARABONBA_PTR_SET_VALUE(task_, task) };
    inline QuotaJobViewMetric& setTask(Task && task) { DARABONBA_PTR_SET_RVALUE(task_, task) };
