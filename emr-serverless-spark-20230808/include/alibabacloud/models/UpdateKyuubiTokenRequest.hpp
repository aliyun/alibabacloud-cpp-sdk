...
namespace Models
{
  ...
  public:
    ...
    virtual bool empty() const override { return this->autoExpireConfiguration_ == nullptr
        && this->memberArns_ == nullptr && this->name_ == nullptr && this->sparkRole_ == nullptr && this->token_ == nullptr && this->regionId_ == nullptr; };
    ...
    inline UpdateKyuubiTokenRequest& setSparkRole(const vector<string> & sparkRole) { DARABONBA_PTR_SET_VALUE(sparkRole_, sparkRole) };
    inline UpdateKyuubiTokenRequest& setSparkRole(vector<string> && sparkRole) { DARABONBA_PTR_SET_RVALUE(sparkRole_, sparkRole) };
