...
namespace Models
{
  ...
  public:
    ...
    inline const Database & body() const { DARABONBA_PTR_GET_CONST(body_, Database) };
    inline Database body() { DARABONBA_PTR_GET(body_, Database) };
    inline GetDatabaseResponse& setBody(const Database & body) { DARABONBA_PTR_SET_VALUE(body_, body) };
    inline GetDatabaseResponse& setBody(Database && body) { DARABONBA_PTR_SET_RVALUE(body_, body) };


  protected:
    std::shared_ptr<map<string, string>> headers_ = nullptr;
    std::shared_ptr<int32_t> statusCode_ = nullptr;
    std::shared_ptr<Database> body_ = nullptr;
  };
