...
namespace Models
{
  ...
  public:
    ...
    inline const vector<Role> & roles() const { DARABONBA_PTR_GET_CONST(roles_, vector<Role>) };
    inline vector<Role> roles() { DARABONBA_PTR_GET(roles_, vector<Role>) };
  ...
  protected:
    ...
    std::shared_ptr<vector<Role>> roles_ = nullptr;
  };
