...
namespace Models
{
  ...
  public:
    ...
    inline const Role & body() const { DARABONBA_PTR_GET_CONST(body_, Role) };
    inline Role body() { DARABONBA_PTR_GET(body_, Role) };
    inline GetRoleResponse& setBody(const Role & body) { DARABONBA_PTR_SET_VALUE(body_, body) };
    inline GetRoleResponse& setBody(Role && body) { DARABONBA_PTR_SET_RVALUE(body_, body) };


  protected:
    std::shared_ptr<map<string, string>> headers_ = nullptr;
    std::shared_ptr<int32_t> statusCode_ = nullptr;
    std::shared_ptr<Role> body_ = nullptr;
  };
