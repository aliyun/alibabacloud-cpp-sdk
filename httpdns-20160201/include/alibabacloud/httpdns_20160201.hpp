...
public:
  ...
  explicit RefreshResolveCacheRequest(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  shared_ptr<string> domainsShrink{};

  RefreshResolveCacheShrinkRequest() {}

  explicit RefreshResolveCacheShrinkRequest(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  map<string, boost::any> toMap() override {
    map<string, boost::any> res;
    if (domainsShrink) {
      res["Domains"] = boost::any(*domainsShrink);
    }
    return res;
  }
  ...
  void fromMap(map<string, boost::any> m) override {
    if (m.find("Domains") != m.end() && !m["Domains"].empty()) {
      domainsShrink = make_shared<string>(boost::any_cast<string>(m["Domains"]));
    }
  }
...
public:
  ...
  RefreshResolveCacheResponseBody() {}

  explicit RefreshResolveCacheResponseBody(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  shared_ptr<map<string, string>> headers{};
  shared_ptr<long> statusCode{};
  shared_ptr<RefreshResolveCacheResponseBody> body{};

  RefreshResolveCacheResponse() {}

  explicit RefreshResolveCacheResponse(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  void fromMap(map<string, boost::any> m) override {
    ...
    if (m.find("body") != m.end() && !m["body"].empty()) {
      if (typeid(map<string, boost::any>) == m["body"].type()) {
        RefreshResolveCacheResponseBody model1;
        model1.fromMap(boost::any_cast<map<string, boost::any>>(m["body"]));
        body = make_shared<RefreshResolveCacheResponseBody>(model1);
      }
    }
  }
...
public:
  ...
  RefreshResolveCacheResponse refreshResolveCacheWithOptions(shared_ptr<RefreshResolveCacheRequest> tmpReq, shared_ptr<Darabonba_Util::RuntimeOptions> runtime);
  RefreshResolveCacheResponse refreshResolveCache(shared_ptr<RefreshResolveCacheRequest> request);
