...
SendMessageResponse Alibabacloud_Cloud-siem20220616::Client::sendMessageWithOptions(shared_ptr<SendMessageRequest> request, shared_ptr<Darabonba_Util::RuntimeOptions> runtime) {
  Darabonba_Util::Client::validateModel(request);
  shared_ptr<map<string, boost::any>> body = make_shared<map<string, boost::any>>(map<string, boost::any>());
  if (!Darabonba_Util::Client::isUnset<long>(request->channelType)) {
    body->insert(pair<string, long>("ChannelType", *request->channelType));
  }
  if (!Darabonba_Util::Client::isUnset<long>(request->receiveUid)) {
    body->insert(pair<string, long>("ReceiveUid", *request->receiveUid));
  }
  if (!Darabonba_Util::Client::isUnset<string>(request->regionId)) {
    body->insert(pair<string, string>("RegionId", *request->regionId));
  }
...
SendMessageResponse Alibabacloud_Cloud-siem20220616::Client::sendMessage(shared_ptr<SendMessageRequest> request) {
  shared_ptr<Darabonba_Util::RuntimeOptions> runtime = make_shared<Darabonba_Util::RuntimeOptions>();
