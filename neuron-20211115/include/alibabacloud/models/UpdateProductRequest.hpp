...
namespace Models
{
  ...
  public:
    ...
    inline const ProductUpdateCmd & getBody() const { DARABONBA_PTR_GET_CONST(body_, ProductUpdateCmd) };
    inline ProductUpdateCmd getBody() { DARABONBA_PTR_GET(body_, ProductUpdateCmd) };
    inline UpdateProductRequest& setBody(const ProductUpdateCmd & body) { DARABONBA_PTR_SET_VALUE(body_, body) };
    inline UpdateProductRequest& setBody(ProductUpdateCmd && body) { DARABONBA_PTR_SET_RVALUE(body_, body) };


  protected:
    shared_ptr<ProductUpdateCmd> body_ {};
  };
