...
namespace Models
{
  ...
  public:
    ...
    inline const vector<UpdateBackupPlanShrinkRequest::Rule> & getRule() const { DARABONBA_PTR_GET_CONST(rule_, vector<UpdateBackupPlanShrinkRequest::Rule>) };
    inline vector<UpdateBackupPlanShrinkRequest::Rule> getRule() { DARABONBA_PTR_GET(rule_, vector<UpdateBackupPlanShrinkRequest::Rule>) };
    inline UpdateBackupPlanShrinkRequest& setRule(const vector<UpdateBackupPlanShrinkRequest::Rule> & rule) { DARABONBA_PTR_SET_VALUE(rule_, rule) };
    inline UpdateBackupPlanShrinkRequest& setRule(vector<UpdateBackupPlanShrinkRequest::Rule> && rule) { DARABONBA_PTR_SET_RVALUE(rule_, rule) };
  ...
  protected:
    ...
    shared_ptr<vector<UpdateBackupPlanShrinkRequest::Rule>> rule_ {};
    // The backup policy. Format: `I|{startTime}|{interval}`. The system runs the first backup job at a point in time that is specified in the `{startTime}` parameter and the subsequent backup jobs at an interval that is specified in the `{interval}` parameter. The system does not run a backup job before the specified point in time. Each backup job, except the first one, starts only after the previous backup job is completed. For example, `I|1631685600|P1D` specifies that the system runs the first backup job at 14:00:00 on September 15, 2021 and the subsequent backup jobs once a day.
    // 
