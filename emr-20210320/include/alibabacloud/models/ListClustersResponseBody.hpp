...
namespace Models
{
  ...
  public:
    ...
    inline const vector<ClusterSummary> & clusters() const { DARABONBA_PTR_GET_CONST(clusters_, vector<ClusterSummary>) };
    inline vector<ClusterSummary> clusters() { DARABONBA_PTR_GET(clusters_, vector<ClusterSummary>) };
    inline ListClustersResponseBody& setClusters(const vector<ClusterSummary> & clusters) { DARABONBA_PTR_SET_VALUE(clusters_, clusters) };
    inline ListClustersResponseBody& setClusters(vector<ClusterSummary> && clusters) { DARABONBA_PTR_SET_RVALUE(clusters_, clusters) };
    ...
    inline ListClustersResponseBody& setMaxResults(int32_t maxResults) { DARABONBA_PTR_SET_VALUE(maxResults_, maxResults) };
  ...
  protected:
    // The clusters.
    std::shared_ptr<vector<ClusterSummary>> clusters_ = nullptr;
