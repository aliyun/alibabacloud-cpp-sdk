...
namespace Models
{
  ...
  public:
    ...
    inline const Gateway & items() const { DARABONBA_PTR_GET_CONST(items_, Gateway) };
    inline Gateway items() { DARABONBA_PTR_GET(items_, Gateway) };
  ...
  protected:
    std::shared_ptr<Gateway> items_ = nullptr;
