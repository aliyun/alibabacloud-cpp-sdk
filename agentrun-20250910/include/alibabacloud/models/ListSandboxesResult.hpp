...
namespace Models
{
  ...
  public:
    ...
    inline const ListSandboxesOutput & data() const { DARABONBA_PTR_GET_CONST(data_, ListSandboxesOutput) };
    inline ListSandboxesOutput data() { DARABONBA_PTR_GET(data_, ListSandboxesOutput) };
  ...
  protected:
    ...
    // 沙箱列表的详细信息
    std::shared_ptr<ListSandboxesOutput> data_ = nullptr;
    // 唯一的请求标识符，用于问题追踪
