...
namespace Models
{
  ...
  public:
    ...
    virtual bool empty() const override { this->authenticationCode1_ != nullptr
    ...
    inline string authenticationCode1() const { DARABONBA_PTR_GET_DEFAULT(authenticationCode1_, "") };
    ...
    inline string authenticationCode2() const { DARABONBA_PTR_GET_DEFAULT(authenticationCode2_, "") };
  ...
  protected:
    ...
    std::shared_ptr<string> authenticationCode1_ = nullptr;
    // The second verification code.
    // 
    // >  You can call the [CreateVirtualMFADevice](https://help.aliyun.com/document_detail/186179.html) operation to create an MFA device and generate a key (value of `Base32StringSeed`). Then, use the key on the Alibaba Cloud app to manually add an MFA device, and obtain the two consecutive verification codes.
    std::shared_ptr<string> authenticationCode2_ = nullptr;
