...
namespace Models
{
  ...
  protected:
    ...
    // - When you enable custom parameters or modify the custom parameters in Parameters, set this parameter to `true`.
    // - When you do not modify the custom parameters in Parameters, do not set this parameter or set it to `false`.
    ...
    //     - The execution interval is based only on the fixed frequency and is not related to the actual execution time of the task. For example, if the command is set to execute every 5 minutes and the task takes 2 minutes to complete, the next round of execution starts 3 minutes after the task is completed.
    ...
    // 
    // - One-time execution at a specified time: The command is executed once at the specified time zone and time point. Format: `at(yyyy-MM-dd HH:mm:ss <time zone>)`, which is `at(year-month-day hour:minute:second <time zone>)`. If no time zone is specified, UTC is used by default. The time zone supports the following three formats:
    ...
    //     - Time zone offset from Greenwich Mean Time: such as `GMT+8:00` (UTC+8) and `GMT-7:00` (UTC-7). When using the GMT format, leading zeros are not supported for the hour value.
    //     - Time zone abbreviation: Only UTC (Coordinated Universal Time) is supported.
    //   For example, to execute once at 10:15 every day in 2022 in China/Shanghai time, use `0 15 10 ? * * 2022 Asia/Shanghai`. To execute every half hour from 10:00 to 11:30 every day in 2022 in UTC+8, use `0 0/30 10-11 * * ? 2022 GMT+8:00`. To execute every 5 minutes from 14:00 to 14:55 every day in October every two years starting from 2022 in UTC, use `0 0/5 14 * 10 ? 2022/2 UTC`.
    // 
    ...
    // The instance ID of the ECS instance or managed instance to add to the scheduled task.
