...
AlivisionImgdupResponse Alibabacloud_Dplus20201216::Client::alivisionImgdupWithOptions(shared_ptr<AlivisionImgdupRequest> request, shared_ptr<Darabonba_Util::RuntimeOptions> runtime) {
  ...
  if (!Darabonba_Util::Client::isUnset<string>(request->picNumList)) {
    body->insert(pair<string, string>("PicNumList", *request->picNumList));
  }
  if (!Darabonba_Util::Client::isUnset<string>(request->picUrlList)) {
    body->insert(pair<string, string>("PicUrlList", *request->picUrlList));
  }
  shared_ptr<Alibabacloud_OpenApi::OpenApiRequest> req = make_shared<Alibabacloud_OpenApi::OpenApiRequest>(map<string, boost::any>({
    {"body", boost::any(Alibabacloud_OpenApiUtil::Client::parseToMap(body))}
  }));
