...
namespace Models
{
  ...
  public:
    ...
    inline const map<string, vector<DataValue>> & getData() const { DARABONBA_PTR_GET_CONST(data_, map<string, vector<DataValue>>) };
    inline map<string, vector<DataValue>> getData() { DARABONBA_PTR_GET(data_, map<string, vector<DataValue>>) };
  ...
  protected:
    ...
    // The returned data.
    shared_ptr<map<string, vector<DataValue>>> data_ {};
