...
class GetRegionListResponseBodyDataRegionList : public Darabonba::Model {
public:
  shared_ptr<string> region{};
  shared_ptr<string> regionDesc{};

  GetRegionListResponseBodyDataRegionList() {}

  explicit GetRegionListResponseBodyDataRegionList(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  virtual ~GetRegionListResponseBodyDataRegionList() = default;
};
class GetRegionListResponseBodyData : public Darabonba::Model {
public:
  shared_ptr<vector<GetRegionListResponseBodyDataRegionList>> regionList{};
  ...
  void fromMap(map<string, boost::any> m) override {
    if (m.find("regionList") != m.end() && !m["regionList"].empty()) {
      if (typeid(vector<boost::any>) == m["regionList"].type()) {
        vector<GetRegionListResponseBodyDataRegionList> expect1;
        for(auto item1:boost::any_cast<vector<boost::any>>(m["regionList"])){
          if (typeid(map<string, boost::any>) == item1.type()) {
            GetRegionListResponseBodyDataRegionList model2;
            model2.fromMap(boost::any_cast<map<string, boost::any>>(item1));
            expect1.push_back(model2);
          }
        }
        regionList = make_shared<vector<GetRegionListResponseBodyDataRegionList>>(expect1);
      }
    }
  }
