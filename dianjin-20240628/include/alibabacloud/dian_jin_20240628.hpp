...
public:
  ...
  explicit UpdateDocumentChunkRequestChunks(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  shared_ptr<vector<UpdateDocumentChunkRequestChunks>> chunks{};
  shared_ptr<string> libraryId{};

  UpdateDocumentChunkRequest() {}

  explicit UpdateDocumentChunkRequest(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  void fromMap(map<string, boost::any> m) override {
    if (m.find("chunks") != m.end() && !m["chunks"].empty()) {
      if (typeid(vector<boost::any>) == m["chunks"].type()) {
        vector<UpdateDocumentChunkRequestChunks> expect1;
        for(auto item1:boost::any_cast<vector<boost::any>>(m["chunks"])){
          if (typeid(map<string, boost::any>) == item1.type()) {
            UpdateDocumentChunkRequestChunks model2;
            model2.fromMap(boost::any_cast<map<string, boost::any>>(item1));
            expect1.push_back(model2);
          }
        }
        chunks = make_shared<vector<UpdateDocumentChunkRequestChunks>>(expect1);
      }
    }
    if (m.find("libraryId") != m.end() && !m["libraryId"].empty()) {
      libraryId = make_shared<string>(boost::any_cast<string>(m["libraryId"]));
...
public:
  ...
  explicit UpdateDocumentChunkResponseBody(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
...
public:
  shared_ptr<map<string, string>> headers{};
  shared_ptr<long> statusCode{};
  shared_ptr<UpdateDocumentChunkResponseBody> body{};

  UpdateDocumentChunkResponse() {}

  explicit UpdateDocumentChunkResponse(const std::map<string, boost::any> &config) : Darabonba::Model(config) {
    fromMap(config);
  };
  ...
  void fromMap(map<string, boost::any> m) override {
    ...
    if (m.find("body") != m.end() && !m["body"].empty()) {
      if (typeid(map<string, boost::any>) == m["body"].type()) {
        UpdateDocumentChunkResponseBody model1;
        model1.fromMap(boost::any_cast<map<string, boost::any>>(m["body"]));
        body = make_shared<UpdateDocumentChunkResponseBody>(model1);
      }
    }
  }
...
public:
  ...
  UpdateDocumentChunkResponse updateDocumentChunkWithOptions(shared_ptr<string> workspaceId,
                                                             shared_ptr<UpdateDocumentChunkRequest> request,
                                                             shared_ptr<map<string, string>> headers,
                                                             shared_ptr<Darabonba_Util::RuntimeOptions> runtime);
  UpdateDocumentChunkResponse updateDocumentChunk(shared_ptr<string> workspaceId, shared_ptr<UpdateDocumentChunkRequest> request);
