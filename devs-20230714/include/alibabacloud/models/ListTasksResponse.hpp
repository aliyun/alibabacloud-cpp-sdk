...
namespace Models
{
  ...
  public:
    ...
    inline const vector<Task> & body() const { DARABONBA_PTR_GET_CONST(body_, vector<Task>) };
    inline vector<Task> body() { DARABONBA_PTR_GET(body_, vector<Task>) };
    inline ListTasksResponse& setBody(const vector<Task> & body) { DARABONBA_PTR_SET_VALUE(body_, body) };
    inline ListTasksResponse& setBody(vector<Task> && body) { DARABONBA_PTR_SET_RVALUE(body_, body) };


  protected:
    std::shared_ptr<map<string, string>> headers_ = nullptr;
    std::shared_ptr<int32_t> statusCode_ = nullptr;
    std::shared_ptr<vector<Task>> body_ = nullptr;
  };
